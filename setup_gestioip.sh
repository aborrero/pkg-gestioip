...
# setup_gestioip.sh VERSION 3.0.3 20140203
...
then    
    ...
    then
	echo "Loadin environment variables from /etc/apache2/envvars"
	echo "Loadin environment variables from /etc/apache2/envvars" >> $SETUP_LOG
...
then
    echo "cp $INSTALL_DIR/apache22/$GESTIOIP_APACHE_CONF /etc/apache2/sites-available/$GESTIOIP_APACHE_CONF.conf" >> $SETUP_LOG
    cp $INSTALL_DIR/apache22/$GESTIOIP_APACHE_CONF /etc/apache2/sites-available/$GESTIOIP_APACHE_CONF.conf >> $SETUP_LOG 2>&1
