...
# ip_update_gestioip_snmp.pl Version 3.0.25
...
my ( $disable_audit, $test, $verbose, $log, $mail, $help, $version_arg, $community_arg, $location_args );
my $config_name="";
my $network_file="";
my $network_list="";
...
my @vigilada_redes=();
my $redes_hash=get_redes_hash_key_red();


if ( $network_file && $network_list ) {
	print "Only on of the option \"network_file\" or \"list_of_networks\" is allowed\n";
	exit 1;
}
...
if ( $network_file || $network_list ) {
	...
	if ( $network_list ) {
		...
		open(NETWORKS,"<$network_file") or die "can not open $network_file: $!\n";
		while (<NETWORKS>) {
			push @pnetworks,"$_";
		}
		close NETWORKS;
...
sub print_help {
	...
	print "-N, --Network_file=networks.list file with the list of networks to process\n";
...
sub get_redes_hash_key_red {
	...
	my $sth = $dbh->prepare("SELECT n.red_num, n.red, n.BM, n.descr, l.loc, l.id, n.vigilada, n.comentario, c.cat, n.ip_version, INET_ATON(n.red), n.rootnet FROM net n, categorias_net c, locations l WHERE c.id = n.categoria AND l.id = n.loc AND n.rootnet = 0 $ip_version_expr") 
		or die "Can not execute statement: $dbh->errstr";
        $sth->execute() or die "Can not execute statement: $dbh->errstr";
		while ( $ip_ref = $sth->fetchrow_hashref ) {
